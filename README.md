...
玻璃厂情绪整合 Demo 是一个面向心理陪伴场景的本地交互原型。用户可以通过摄像头镜像、语音倾诉或文字输入表达当下状态，系统会将表达内容转化为情绪意象、玻璃碎片、金缮作品和 15 秒情绪音乐，形成一次完整的“看见-表达-重组-安放”体验。

本项目为黑客松原创增量开发代码，核心流程、前端交互、Node 本地服务、语音转写接入、意象提炼接口与音乐生成模块均由本团队在比赛期间整合实现。

## 提交材料清单

| 材料 | 状态 | 说明 |
|---|---|---|
| 代码仓库链接 | 已提供 | GitHub 仓库包含完整源码、运行说明和依赖说明 |
| README.md | 已提供 | 当前文档，包含项目介绍、运行方式、依赖清单和原创声明 |
| 演示文档 | 需随平台提交 | 建议 8-10 页 PPT/PDF，覆盖问题定义、解决方案、技术架构、商业模式和团队分工 |
...
本仓库提交内容为本团队在黑客松期间围绕“情绪表达与心理陪伴”主题完成的原创增量开发。项目未直接复制已有完整开源项目作为参赛作品；如使用第三方库或模型服务，均仅作为基础能力依赖，并在代码和文档中保留其调用边界。
...
根据黑客松提交规则，演示文档建议控制在 8-10 页：

1. 项目名称与一句话介绍
2. 问题定义：用户在情绪表达中的困难
3. 解决方案：玻璃厂情绪整合体验
4. 核心流程：镜像、倾诉、意象、金缮、音乐
5. 技术架构：浏览器、Node 服务、Python 音乐模块、LLM/STT 代理
6. 产品亮点：隐私、本地识别、多模态表达、可降级体验
7. 商业模式：心理陪伴、疗愈内容、线下活动或 B 端合作
8. 团队分工：产品、前端、后端、算法/音乐、设计
9. Demo 截图或运行效果
10. 后续规划
